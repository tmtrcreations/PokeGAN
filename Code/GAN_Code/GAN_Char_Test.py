...
class GAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # Load the dataset
        X_train = 255*np.ones((batch_size, self.img_rows, self.img_cols, self.channels))
        for img_ind in range(1, batch_size+1):
            img = Image.open('../../Images/Training_Images/Single_Sprites/img9.png')
        ...
        for epoch in range(epochs):
            ...
            imgs = X_train
            imgs = imgs + (0.5*(np.cos(np.pi*epoch/60000)+1)/2)*np.random.normal(0, 1, imgs.shape)
...
if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=60000, batch_size=32, sample_interval=200)
